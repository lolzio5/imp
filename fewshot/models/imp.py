...
class IMPModel(Protonet):
    ...
    def _add_cluster(self, nClusters, protos, radii, cluster_type='unlabeled', ex = None):
        ...
        d_radii = torch.ones(bsize, 1).cuda().detach()
        ...
        if ex is None:
            new_proto = self.base_distribution.cuda()
        else:
    ...
    def estimate_lambda(self, tensor_proto, semi_supervised):
        ...
        rho = rho.mean().item()  # Convert to Python scalar for numpy compatibility
    ...
    def loss(self, logits, targets, labels):
        ...
        inputs:
            ...
            targets [B, N, nClusters] boolean mask of valid clusters
            labels [nClusters] class label for each cluster
        outputs:
            weighted cross entropy such that we have an "or" function
            across prototypes in the class of each query
        """
        # Squeeze batch dimension for processing (B=1)
        logits_2d = logits.squeeze(0)  # [N, nClusters]
        targets_2d = targets.squeeze(0)  # [N, nClusters]

        # determine index of closest in-class prototype for each query
        target_logits = torch.ones_like(logits_2d) * float('-Inf')
        target_logits[targets_2d.bool()] = logits_2d.detach()[targets_2d.bool()]
        _, best_targets = torch.max(target_logits, dim=-1)
        # mask out everything...
        weights = torch.zeros_like(logits_2d)
        # ...then include the closest prototype in each class and unlabeled)
        unique_labels = np.unique(labels.cpu().numpy())
        for l in unique_labels:
            class_mask = (labels == l)
            class_logits = torch.ones_like(logits_2d) * float('-Inf')
            class_logits[:, class_mask] = logits_2d[:, class_mask].detach()
            _, best_in_class = torch.max(class_logits, dim=-1)
            weights[range(0, logits_2d.size(0)), best_in_class] = 1.
        loss = weighted_loss(logits_2d, best_targets, weights)
        return loss.mean()
    ...
    def forward(self, sample, super_classes=False):
        ...
        for ii in range(self.config.num_cluster_steps):
            ...
            for i, ex in enumerate(h_train[0]):
                ...
                if (torch.min(distances) > lamda):
                    ...
                    support_labels = torch.cat([support_labels, batch.y_train[0, i].detach().unsqueeze(0)], dim=0)

            #perform partial reassignment based on newly created labeled clusters
            if nClusters > nInitialClusters:
                support_targets = (batch.y_train.detach()[0, :, None] == support_labels).unsqueeze(0)
                prob_train = assign_cluster_radii_limited(tensor_proto.cuda(), h_train, radii, support_targets)
            ...
            if batch.x_unlabel is not None:
                ...
                prob_all = torch.cat([prob_train.detach().cuda(), prob_unlabel_nograd], dim=1)
        ...
        support_targets = (labels[0, :, None] == support_labels).unsqueeze(0)
        loss = self.loss(logits, support_targets, support_labels)

        # map support predictions back into classes to check accuracy
        _, support_preds = torch.max(logits.squeeze(0).detach(), dim=1)
